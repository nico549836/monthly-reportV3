...
function isBeforeStartTime(startDate) {
  const now = new Date();
...
function renderReportTable(plans) {
  ...
  tbody.innerHTML = pageData.map(plan => {
    ...
    const beforeStart = isBeforeStartTime(plan.startDate);
    const hasData = Store.hasUserReport(plan.id, currentUserId);

    let statusHtml;
    if (beforeStart) {
      statusHtml = `<span class="tag tag-info">未开始</span>`;
    } else if (hasData) {
    ...
    const disabled = beforeStart ? 'disabled style="opacity:0.5;cursor:not-allowed;"' : '';
    ...
    return `
      <tr>
        ...
        <td>${statusHtml}</td>
        <td><button class="btn btn-link" ${disabled} onclick="openReportModal('${plan.id}')">填报</button></td>
      </tr>
    `;
  }).join('');
    ...
    return `
      <tr>
        ...
        <td>
          ...
          <button class="btn btn-link" style="color:var(--warning)" onclick="openCancelModal('${plan.id}')" ${plan.status === 'cancelled' ? 'disabled style="opacity:0.5"' : ''}>作废</button>
...
function openAddPlanModal() {
  ...
  document.getElementById('plan-form-start').value = '';
...
function openEditPlanModal(planId) {
  ...
  document.getElementById('plan-form-start').value = plan.startDate;
...
function submitPlan() {
  ...
  const startDate = document.getElementById('plan-form-start').value;
  const fields = [];
  ...
  const planData = { name, month, fields, startDate };
